...
def plot_learning_curve(logfile_dir, result_lists):
    ...
    # Manually set the x-ticks to only use integer epochs
    epochs = list(range(1, len(result_lists['train_acc']) + 1))
    xticks = list(range(0, len(result_lists['train_acc']), 5))

    # Training accuracy
    axs[0, 0].plot(epochs,result_lists['train_acc'], label='Train Accuracy')
    axs[0, 0].set_xticks(xticks)  # Set x-axis ticks to integer epochs
    ...
    # Training loss
    axs[0, 1].plot(epochs,result_lists['train_loss'], label='Train Loss')
    axs[0, 1].set_xticks(xticks)  # Set x-axis ticks to integer epochs
    ...
    # Validation accuracy
    axs[1, 0].plot(epochs,result_lists['val_acc'], label='Validation Accuracy', color='darkviolet')
    axs[1, 0].set_xticks(xticks)  # Set x-axis ticks to integer epochs
    ...
    # Validation loss
    axs[1, 1].plot(epochs,result_lists['val_loss'], label='Validation Loss', color='darkviolet')
    axs[1, 1].set_xticks(xticks)  # Set x-axis ticks to integer epochs
...
def train(model, train_loader, val_loader, logfile_dir, model_save_dir, criterion, optimizer, scheduler, device):
    ...
    for epoch in range(cfg.epochs):
        ...
        if is_better:
            ...
            torch.save(model.state_dict(), os.path.join(model_save_dir, f'{cfg.model_type}_best.pth'))
            best_acc = val_acc
