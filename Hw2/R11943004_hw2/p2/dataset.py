...
def get_dataloader(dataset_dir, batch_size=1, split='test'):
    ...
    if split == 'train':
        transform = transforms.Compose([
            ...
            transforms.RandomRotation(15),  # randomly rotate image by 10 degrees
...
class CIFAR10Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'test':
            # If it's the test split, return the image only
            return {'images': image}  # No labels to return
        else:
            # For training and validation, return both image and label
