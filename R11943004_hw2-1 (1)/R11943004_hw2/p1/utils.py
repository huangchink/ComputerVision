...
def get_tiny_images(img_paths):
    ...
    for path in img_paths:
        ...
        tiny_img = img.resize((16, 16), Image.ANTIALIAS)
        tiny_img1D = np.array(tiny_img).flatten()
        tiny_imgs.append(tiny_img1D)

    tiny_imgs = np.array(tiny_imgs)
    
    tiny_imgs = tiny_imgs - np.mean(tiny_imgs, axis=1, keepdims=True)
    tiny_img_feats = tiny_imgs / np.linalg.norm(tiny_imgs, axis=1, keepdims=True)
...
def build_vocabulary(img_paths, vocab_size=400):
    ...
    for path in img_paths:
        ...
        #print(frames.shape)
        descriptor_128=descriptors[::8]
        for descriptor_sample in descriptor_128:
             vocabularies_list.append(descriptor_sample)
    
    #print("np.matrix(X).shape", np.matrix(X).shape)
    vocabularies_list = np.array(vocabularies_list).astype('float32')
    vocabularies = kmeans(vocabularies_list, num_centers=vocab_size,initialization="PLUSPLUS")    
    ##################################################################################
    #                                END OF YOUR CODE                                #
    ##################################################################################
    # return vocab
...
def get_bags_of_sifts(img_paths, vocab):
    ...
    for path in img_paths:
        ...
        descriptor_128=descriptors[::8]
        for descriptor_sample in descriptor_128:
             img_feat.append(descriptor_sample)
    
        img_feat_np=np.array(img_feat)
        #histogram
        dist = cdist(img_feat_np,vocab)  
...
def nearest_neighbor_classify(train_img_feats, train_labels, test_img_feats):
    ...
    #print(label.shape)
    dis = cdist(test_img_feats, train_img_feats, 'minkowski',p=1)
    test_predicts = []
    
    for row in dis:
        kmin = np.argpartition(row, k)[:k]
        nearest_labels = train_labels[kmin]
