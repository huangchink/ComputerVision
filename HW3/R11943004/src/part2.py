    ...
    while (video.isOpened()):
        ...
        if ret:  ## check whethere the frame is legal, i.e., there still exists a frame
            ...
            if ids is not None:
                ...
                corner=np.round(corners[0][0])#我們目標的marker是id 0。
                # TODO: 2.find homograpy
                # function call to solve_homography()
                H1 = solve_homography(ref_corns,corner)
                ymin,ymax,xmin,xmax = np.min(corner,axis=0)[1] , np.max(corner,axis=0)[1],np.min(corner,axis=0)[0] , np.max(corner,axis=0)[0]
                # TODO: 3.apply backward warp
                # function call to warping()
                frame = warping(ref_image, frame, H1, ymin, ymax, xmin, xmax, direction='b')
